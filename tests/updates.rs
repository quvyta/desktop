//! The family's update notice: qdesk asks once at start whether a newer version is out while the
//! family's switch is on, says so in the corner when one is, and the switch on the Settings screen
//! turns the question off for the whole family.
//!
...
/// The family's folders of a test named `what`, empty, so the notice starts on as it does on a
/// machine that never chose.
fn family(what: &str) -> UpdateFolders {
...
fn clean(folders: &UpdateFolders) {
    let root: PathBuf = folders.config.parent().expect("the family's root").to_path_buf();
    let _ = std::fs::remove_dir_all(root);
}
...
/// qdesk started over the family's `folders`, as a person starts it, with the usual floor.
...
/// Moves the switch of the family's update notice where it is drawn: a switch is colour alone
...
fn a_newer_version_is_asked_for_once_and_said_in_the_corner() {
    ...
    let mut same = started(Some(family("same")));
...
#[test]
fn the_switch_on_the_settings_screen_turns_the_question_off_for_the_family() {
...
#[test]
fn without_the_familys_folders_nothing_is_asked_and_no_switch_is_shown() {
