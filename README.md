...
This is **0.1.10**, the eleventh release. It is a young program: what is on this page is what it does on screen today, and the end of the page says plainly what it does not do yet — there is no file or picture viewer yet, and the programs in its windows do not survive a dropped connection.
...
- **A Settings screen.** The appearance every Quvyta application shares, with the same rows as the others: language, theme, icons and reduced motion, each with a box saying whether a change goes to every Quvyta application or stays on the desktop alone, then the pillar and the Quvyta-wide update notice; which edge the dock sits on and the colour, pattern and picture of the floor; whether a folder opens in qexp, the ecosystem's file explorer, when it is installed, or in Files; how a window is dragged and how many frames a second are drawn; how many lines a terminal window remembers; and the folders entries are read from, with anything that could not be read. The shared look is kept in `~/.config/quvyta/quvyta.conf`, and the desktop's own settings, with anything chosen for the desktop alone, in `desktop.conf` beside it; only what you chose is ever written. An open desktop follows both: another Quvyta application switching the shared theme changes the desktop at once, unless the desktop keeps a theme of its own, and a change saved by hand or by `qdesk wallpaper` is applied without a restart, and a line it cannot use is pointed out.
