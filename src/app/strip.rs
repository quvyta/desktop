...
impl Desk {
    ...
    /// What a press on the strip's item of `kind` does: the processor and the memory open the
    /// machine's process monitor when it has one. The tmux item answers with its menu instead.
    pub(super) fn chip_press(&self, kind: Kind) -> Option<Msg> {
        match kind {
            Kind::Cpu | Kind::Memory => self.monitor.as_ref().map(|_| Msg::Monitor),
            Kind::Tmux | Kind::Network | Kind::Battery => None,
        }
    }

    /// The menu a click of either button on the strip's item of `kind` opens: the tmux sessions,
    /// each one attaching a new window to it. The other items have none.
    ...
    ///
    /// The window starts without `TMUX`: a qdesk started inside tmux passes it on, and tmux then
    /// refuses to attach, saying sessions should be nested with care.
